...
#define CMAX 10
#define VMAX 10
int NC, NV, SEMOTIMIZAR,P1,P2,XERR;
double TS[CMAX][VMAX];
void Data() {
double R1,R2;
char R;
int I,J;
printf("\n SIMPLEX \n\n");
printf(" MAXIMIZAR(X) OU MINIMIZAR(N)? "); scanf("%c", &R);
printf("\n NUMERO DE VARIAVEIS? ");
scanf("%d", &NV);
printf("\n NUMERO DE RESTRICOES? "); scanf("%d", &NC);
if (R == 'X' || R=='x')
R1 = 1.0;
else
R1 = -1.0;
printf("\n VALORES DA FUNCAO OBJETIVA:\n");
for (J = 1; J<=NV; J++) {
printf(" X%d = ", J); scanf("%lf", &R2);
TS[1][J+1] = R2 * R1;
}
printf(" NAO NEGATIVIDADE >= "); scanf("%lf", &R2);
TS[1][1] = R2 * R1;
for (I = 1; I<=NC; I++) {
printf("\n RESTRICAO %d:\n", I);
for (J = 1; J<=NV; J++) {
